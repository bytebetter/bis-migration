...
<#
  รัน migration pacs_sync_patient (MSSQL -> Postgres / Directus)

  ตาราง log ไม่มี PK — insert ทุกแถว ถ้าจะรันทับชุดเดิมให้ TRUNCATE public.pacs_sync_patient
  แล้วลบ checkpoints/ ก่อน ไม่งั้นได้แถวซ้ำ
#>

param(
  [string] $ConfigPath = "..\..\migration.config.local.json",
  [string] $Profile = "pacs_sync_patient",
