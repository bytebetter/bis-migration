...
async function main() {
  ...
  const migration = mergeMigrationWithCli(config?.migration, "mam");
  ...
  try {
    ...
    try {
      ...
      const fieldIssueLogPath = path.join(
        logsDir,
        `migration-field-issues-mammogram-${nowStamp()}.json`,
      );
      let chunkIndex = 0;
      ...
      if (fieldIssueAcc.totalFieldIssueCount > 0) {
        const payload = buildFieldIssueLogPayload(fieldIssueAcc, {
          migrationKey: KEY,
          logType: "mammogram_field_issues",
          recordIdKey: "exam_id",
          buildRecord: (rec) => ({
      ...
      runLog.repairSummary = finishRepairRunSummary(KEY, REPAIR_SPEC_MAM, repairRun, {
        fieldIssueAcc,
      });
      runLog.status = "success";
    } finally {
