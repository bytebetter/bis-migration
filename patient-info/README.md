...
```
scripts/db-migration/patient-info/
├── README.md                 ← คู่มือนี้
...
```powershell
cd .\scripts\db-migration\patient-info\tools
npm install
```
...
```powershell
.\scripts\db-migration\patient-info\run-migrate-clone.ps1 -CsvPath .\scripts\db-migration\patient-info\imports\patient_info.csv
```

ถ้าต้องการ **ล้าง `public.patient_info` ใน clone ก่อน** (รอบทดสอบ):

```powershell
.\scripts\db-migration\patient-info\run-migrate-clone.ps1 -CsvPath .\scripts\db-migration\patient-info\imports\patient_info.csv -TruncateFirst
```
...
1. **Export จาก MSSQL**  
   ...
   วางไฟล์ที่ `scripts/db-migration/patient-info/imports/` เช่น `patient_info.csv`
...
3. **สร้าง staging**  
   ...
   Get-Content .\scripts\db-migration\patient-info\sql\01_create_staging.sql -Raw | kubectl -n default exec -i postgresql-0 -- psql -U devuser -d bisinfo_dev_clone
   ```
...
4. **โหลด CSV เข้า staging**  
   ...
   ```powershell
   Get-Content .\scripts\db-migration\patient-info\imports\patient_info.csv -Raw | kubectl -n default exec -i postgresql-0 -- psql -U devuser -d bisinfo_dev_clone -c "COPY migrate_stg.patient_info_mssql FROM STDIN WITH (FORMAT csv, HEADER true, ENCODING 'UTF8');"
   ```
...
5. **Insert เข้า `public.patient_info`**
   ...
   ```powershell
   Get-Content .\scripts\db-migration\patient-info\sql\02_insert_into_clone_patient_info.sql -Raw | kubectl -n default exec -i postgresql-0 -- psql -U devuser -d bisinfo_dev_clone
   ```

6. **Verify**

   ```powershell
   Get-Content .\scripts\db-migration\patient-info\sql\04_verify_clone.sql -Raw | kubectl -n default exec -i postgresql-0 -- psql -U devuser -d bisinfo_dev_clone
   ```
