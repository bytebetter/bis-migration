/**
 * สร้าง config สำหรับ sql.connect — ใช้ร่วมกับ snapshot count / migrate
 */
...
export function buildMssqlConfig(sourceConfig = {}) {
  const timeouts = tediousOptionsFromSource(sourceConfig);
  if (sourceConfig.mssqlUrl) {
    const base = parseMssqlUrl(String(sourceConfig.mssqlUrl));
    return {
      ...base,
      options: {
        ...base.options,
