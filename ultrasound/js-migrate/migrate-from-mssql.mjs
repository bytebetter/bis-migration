...
import pg from "pg";
import { createMssqlUltrasoundSelectBundle } from "./mssqlUltrasoundSelect.mjs";
import {
...
async function main() {
  ...
  const sortBundle = await setupCreatedDateMigrationSort(pool, {
    ...
    sourceTable,
    tableLabel: KEY,
    createSelectBundle: createMssqlUltrasoundSelectBundle,
  });
  ...
  try {
    ...
    try {
      ...
      const fieldIssueLogPath = path.join(
        logsDir,
        `migration-field-issues-ultrasound-${nowStamp()}.json`,
      );
      let chunkIndex = 0;
      ...
      if (fieldIssueAcc.totalFieldIssueCount > 0) {
        const payload = buildFieldIssueLogPayload(fieldIssueAcc, {
          migrationKey: KEY,
          logType: "ultrasound_field_issues",
          recordIdKey: "exam_id",
          buildRecord: (rec) => ({
