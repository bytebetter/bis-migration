...
import pg from "pg";
import { createMssqlMamCalSelectBundle } from "./mssqlMamCalSelect.mjs";
...
async function main() {
  ...
  const sortBundle = await setupCreatedDateMigrationSort(pool, {
    ...
    sourceTable,
    tableLabel: KEY,
    createSelectBundle: createMssqlMamCalSelectBundle,
  });
  ...
  try {
    ...
    try {
      ...
      while (true) {
        ...
        if (sortBundle.createdDateColumn) {
          compositeKs = advanceExamChildCompositeKeyset(last, "described_cal_id");
        } else {
