...
import pg from "pg";
import { createMssqlMamMassSelectBundle } from "./mssqlMamMassSelect.mjs";
...
const COMPOSITE_FIELD_ISSUE = buildCompositeStagingFieldIssueConfig(
  "described_mass_id",
  "mammogram_mass",
  "migrate_stg.mammogram_mass_mssql",
);
...
async function main() {
  ...
  const sortBundle = await setupCreatedDateMigrationSort(pool, {
    ...
    sourceTable,
    tableLabel: KEY,
    createSelectBundle: createMssqlMamMassSelectBundle,
  });
  ...
  try {
    ...
    try {
      ...
      const fieldIssueLogPath = path.join(
        logsDir,
        `migration-field-issues-mammogram_mass-${nowStamp()}.json`,
      );
      let chunkIndex = 0;
      ...
      if (fieldIssueAcc.totalFieldIssueCount > 0) {
        const payload = buildFieldIssueLogPayload(fieldIssueAcc, {
          migrationKey: KEY,
          logType: "mammogram_mass_field_issues",
